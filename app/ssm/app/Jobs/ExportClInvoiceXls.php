...
class ExportClInvoiceXls implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        $sheet->setCellValue('A1', "Client Invoice Report Of $name" )
        ->mergeCells('A1:H1')
        ...
        $sheet->setCellValue('A3', 'Date')
              ->setCellValue('B3', 'Invoice ID')
              ->setCellValue('C3', 'Challan ID')
              ->setCellValue('D3', 'Payslip ID')
              ->setCellValue('E3', 'Amount')
              ->setCellValue('F3', 'Due Amount')
              ->setCellValue('G3', 'Paid Amount')
              ...
              foreach ($invoices as $invoice) {
                ...
                $sheet->setCellValue('A' . $rowNumber, $invoice->created_at->format('d-m-Y')) // Format date as d-m-Y
                        ->setCellValue('B' . $rowNumber, $invoice->uid)
                        ->setCellValue('C' . $rowNumber, $invoice->challan_id)
                        ->setCellValue('D' . $rowNumber, $payslipid)
                        ->setCellValue('E' . $rowNumber, $invoice->total_amount_including_gst ? '₹'.$invoice->total_amount_including_gst : '')
                        ->setCellValue('F' . $rowNumber, $invoice->dueamount ? '₹'.$invoice->paidamount : '')
                        ->setCellValue('G' . $rowNumber, $invoice->dueamount ? '₹'.$invoice->paidamount : '')
                        ->setCellValue('H' . $rowNumber, $invoice->status);
